    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Layer(float2 UV, float t){
                ...
                float w = UV.y * 10;
                float x = (n - .5)*.8;
                x += (.47-abs(x)) * sin(3*w)*pow(sin(w), 6)*.45;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                drops += Layer(i.uv * 1.2 + 7.5, t);
                drops += Layer(i.uv * 1.34 + 8, t);

                float fade = 1-saturate(fwidth(i.uv)*70);
                float blur = _Blur * 7 * (1-drops.z * fade * 2);
